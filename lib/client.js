...
client.prototype._sendReplyMessage = function _sendReplayMessage(delay, channel, message, msgId, fn) {
	// Promise a result..
	return new Promise((resolve, reject) => {
		// Make sure the socket is opened and not logged in as a justinfan user..
		if(!this._isConnected()) {
		...
		if(message.length >= 500) {
			...
			setTimeout(() => {
				this._senReplydMessage(delay, channel, msg[1], () => {});
			}, 350);
		}

		this.ws.send(`@reply-parent-msg-id=${msgId} PRIVMSG ${chan} :${message}`);
