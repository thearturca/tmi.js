...
module.exports = {
	...
	// Send reply to message on a channel..
	reply(channel, message, user) {
		channel = _.channel(channel);
		let msgId = user.id;
		if (user["reply-parent-msg-id"] !== undefined){
			msgId = user["reply-parent-msg-id"];
		}
		...
		// Send the command to the server and race the Promise against a delay..
		return this._sendReplyMessage(this._getPromiseDelay(), channel, message, msgId, (resolve, _reject) => {
			// At this time, there is no possible way to detect if a message has been sent has been eaten
			// by the server, so we can only resolve the Promise.
			resolve([ channel, message ]);
